...
Int_t ihcalhightriggered( TString filename )
{
...
6 578 5 467
  ...
  Int_t bottom = (i/4)*4;
    ...
    if ( h != 0 && h->GetEntries() > 20 ) {
      h->Fit("landau");
      TF1 *myfit = (TF1*) h->GetFunction("landau");
      if ( myfit != 0 ) {
	mpv[i]  = myfit->GetParameter(1);
	dmpv[i] = myfit->GetParError(1);
      }
    }
  ...
  TString pdffile = "ihcalhightriggered_";
