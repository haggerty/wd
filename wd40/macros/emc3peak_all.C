...
Int_t emcpeak( TString filename, Int_t column = -1 )
{
  ...
  Int_t rn;
  W->SetBranchAddress("runnumber",&rn);
  W->GetEntry(0);

  ofstream sumfile;
  sumfile.open("emcpeak_all.txt", std::ofstream::app);

  Int_t tower;
  Int_t can;
  for (Int_t i = 0; i < 64; i++ ) {
    if ( (channel_to_canvas_odd(i)-1)%8 == column || column < 0 ) {
      tower = 64 - channel_to_canvas_odd(i);
      can = channel_to_canvas_odd(i);
      std::cout 
	<< rn << " "
	//	<< i << " " 
	<< tower << " " 
  ...
  TString pdffilename = "emcpeak_all_";
